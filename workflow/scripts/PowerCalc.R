...
# }
# 
Var_Tbl <- readRDS('../../scratch/Burden_False_SIFT_False_Del_False_HomoOnly_True_HetThresh_10/Mtb_Var_Tbl.rds')
...
Case_CTL_Ratio <- c(0.005,0.015,0.025,0.05)
MAC_Thresh <- round(Case_CTL_Ratio * nrow(Var_Tbl))
...
pw <- lapply(1:length(MAC_Thresh),function(i) genpwr.calc(calc = "power", model = "logistic", ge.interaction = NULL,
                  ...
                  MAF=seq(0.05, 0.25, 0.01), OR=c(5,15,25),Alpha=Alpha[i],
...
p <- ggplot2::ggplot(aes(x=MAF,y=Power,color = factor(Case.Rate)),data = pw) + geom_line() +
  facet_grid(~OR,labeller = label_both) + labs(color = 'M.tb MAF') + xlab('Human MAF')


# pw <- lapply(1:length(MAC_Thresh),function(i) genpwr.calc(calc = "power", model = "logistic", ge.interaction = NULL,
#                                                                             N=990, Case.Rate=Case_CTL_Ratio[i], k=NULL,
#                                                                             MAF=seq(0.05, 0.25, 0.01), OR=c(15,25),Alpha=5e-8,
#                                                                             True.Model=c( "Additive"),
#                                                                             Test.Model=c("Additive")))
# pw <- lapply(pw,function(x) {
#   colnames(x)[9] <- 'Power'
