...
#' @export
make_combined_plot = function (merged, title1, title2, ld_fn1,ld_fn2, chr, snp = NULL,
                               combine = TRUE, legend = TRUE,
                               legend_position = c('bottomright','topright','topleft'),
                               lz_ylab_linebreak=FALSE,lead_snp = T) {
  
  snp = get_lead_snp(merged, snp)
  # print(sprintf("INFO - %s", snp))
  
  if(lead_snp){
    color1 = assign_color(merged$rsid, snp, ld_fn1)
    color2 = assign_color(merged$rsid, snp, ld_fn2)
    shape1 = ifelse(merged$rsid == snp, 23, 21)
    names(shape1) = merged$rsid
    shape2 = ifelse(merged$rsid == snp, 23, 21)
    names(shape2) = merged$rsid
    size1 = ifelse(merged$rsid == snp, 3, 2)
    names(size1) = merged$rsid
    size2 = ifelse(merged$rsid == snp, 3, 2)
    names(size2) = merged$rsid
    ...
    merged$logp1 <- -log10(merged$pval1)
    merged$logp2 <- -log10(merged$pval2)
    
    merged <- add_label(merged,snp)
    merged1 <- add_label(merged,snp)
    merged2 <- add_label(merged,snp)
    
  }else{
    color1 = assign_color(merged$rsid, merged$rsid[which.min(merged$pval1)], ld_fn1)
    color2 = assign_color(merged$rsid, merged$rsid[which.min(merged$pval2)], ld_fn2)
    shape1 = ifelse(merged$rsid == merged$rsid[which.min(merged$pval1)], 23, 21)
    names(shape1) = merged$rsid
    shape2 = ifelse(merged$rsid == merged$rsid[which.min(merged$pval2)], 23, 21)
    names(shape2) = merged$rsid
    size1 = ifelse(merged$rsid == merged$rsid[which.min(merged$pval1)], 3, 2)
    names(size1) = merged$rsid
    size2 = ifelse(merged$rsid == merged$rsid[which.min(merged$pval2)], 3, 2)
    names(size2) = merged$rsid
    ...
    merged <- add_label(merged,snp)
    merged1 <- add_label(merged,merged$rsid[which.min(merged$pval1)])
    merged2 <- add_label(merged,merged$rsid[which.min(merged$pval2)])
    
  }

  p1 = ggplot2::ggplot(aes(x=logp2,y=logp1,shape=rsid,size=rsid),data = merged) + geom_point() + xlab('GWAS -log10(P)') + ylab('eQTL -log10(P)') + 
    scale_shape_manual(values = shape, guide = "none") + scale_size_manual(values=size,guide='none') +  ggrepel::geom_text_repel(aes(label=label))
  
  # make_scatterplot(merged, title1, title2, color = 'blue',
  #                       shape, size, legend=F, legend_position)
  
  metal1 = merged1 %>% dplyr::select(rsid,logp1,chr=chr1,pos=pos1,label)
  colnames(metal1)[which(colnames(metal1) == 'logp1')] = 'logp'
  p2 = make_locuszoom(metal1, title1, chr, color1, shape1, size1, lz_ylab_linebreak)
  if (legend == TRUE) {
    legend_position = match.arg(legend_position)
    if (legend_position == 'bottomright'){
      legend_box = data.frame(x = 0.8, y = seq(0.4, 0.2, -0.05))
  ...
  metal2 = merged2 %>% dplyr::select(rsid,logp2,chr=chr2,pos=pos2,label)
  colnames(metal2)[which(colnames(metal2) == 'logp2')] = 'logp'
  p3 = make_locuszoom(metal2, title2, chr, color2, shape2, size2, lz_ylab_linebreak)
  if (legend == TRUE) {
    legend_position = match.arg(legend_position)
    if (legend_position == 'bottomright'){
      legend_box = data.frame(x = 0.8, y = seq(0.4, 0.2, -0.05))
...
locuscompare = function(in_fn1, in_fn2, marker_col1 = "rsid", pval_col1 = "pval",
                        ...
                        legend_position = c('bottomright','topright','topleft'),
                        lz_ylab_linebreak = FALSE, genome = c('hg19','hg38'),ld_fn1,ld_fn2) {
  d1 = in_fn1
  d2 = in_fn2
  ...
  snp = get_lead_snp(merged, snp)
  p = make_combined_plot(merged, title1, title2, ld_fn1,ld_fn2, chr, snp, combine,
