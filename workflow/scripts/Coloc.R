...
RunColoc <- function(eqtl_sumstats,gwas_sumstats,out_path,eQTL_POP = 'EUR',method = 'Coloc'){
  if(method == 'Susie'){
    system(glue::glue("mkdir -p {out_path}/tmp/"))
    GWAS_SNP_List <- gwas_sumstats$rsid
    write(GWAS_SNP_List,file = glue::glue("{out_path}/tmp/GWAS_SNPs"))
    bfile_path <- '../../scratch/Burden_False_SIFT_False_Del_False_HomoOnly_True_HetThresh_10/LINEAGE_ALL/TB_DAR_G2G'
    system(glue::glue("/home/zmxu/Software/plink --bfile {bfile_path} --extract {out_path}/tmp/GWAS_SNPs --make-bed --out {out_path}/tmp/GWAS_LD_BFILE"))
    system(glue::glue("/home/zmxu/Software/plink --bfile {out_path}/tmp/GWAS_LD_BFILE --r square --out {out_path}GWAS_LD"))
    ...
    EQTL_Samples <- dplyr::filter(Sample_Manifest_1KG,SuperPOP == eQTL_POP)$`Individual ID`
    data.table::fwrite(data.frame(FID=EQTL_Samples,IID=EQTL_Samples),file = glue::glue("{out_path}/tmp/EQTL_Samples"),row.names = F,col.names = F,sep = '\t')
    
    bfile_path_1KG <- '../../data/1000_Genomes/joined.1000genomes'
    system(glue::glue("/home/zmxu/Software/plink --bfile {bfile_path_1KG} --extract {out_path}/tmp/EQTL_SNPs --keep {out_path}/tmp/EQTL_Samples --make-bed --out {out_path}/tmp/EQTL_LD_BFILE"))
    system(glue::glue("/home/zmxu/Software/plink --bfile {out_path}/tmp/EQTL_LD_BFILE --r square --out {out_path}EQTL_LD"))
    ...
    eqtl_sumstats <- eqtl_sumstats %>% dplyr::filter(rsid %in% rownames(eQTL_LD))
    ...
    eQTL_Susie <- tryCatch(runsusie(eQTL_dataset),error = function(e) return(NULL))
    GWAS_Susie <- runsusie(gwas_dataset)
    
    susie.res <- NULL
    if(!is.null(eQTL_Susie)){
      if(requireNamespace("susieR",quietly=TRUE)) {
        susie.res=tryCatch(coloc.susie(eQTL_Susie,GWAS_Susie),error = function(e) return(NULL))
      }
    }
    saveRDS(list(result=susie.res,
...
RunAll <- function(GWAS_Path,Target_Gene,method = 'Coloc'){
  offset <- 200000
  ...
  PP <- lapply(1:nrow(tabix_paths),function(i){
    ...
    return(RunColoc(EQTL_stats,GWAS_stats,out_path,method = method))
  })
...
# RunAll('../../results/Burden_False_SIFT_False_Del_False_HomoOnly_True_HetThresh_10/PLINK/PC_3_pPC_0/Stratified_False/LINEAGE_ALL/Rv2348c_Rv2348c:2626678:p.Ile101Met.Rv2348c_Rv2348c:2626678:p.Ile101Met.glm.logistic.hybrid.gz','PRDM15',method = 'Coloc')
