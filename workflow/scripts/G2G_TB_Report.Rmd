...
platform <- sapply(G2G_Obj$both_IDs_to_keep$ALL$FAM_ID,function(x) strsplit(x=x,split = '_')[[1]][1])
platform[platform=='Batch1'] <- 'Genotyping'
platform[platform=='Batch2'] <- 'Genotyping'
pheno <- cbind(pheno,data.frame(platform = platform))

ggplot2::ggplot(pheno) + geom_point(aes(x=PC1,y=PC2,color = platform)) + 
  labs(x = paste0('Host PC1 (',round(host_PC_eigenval$V1[1] / sum(host_PC_eigenval$V1) * 100,2),'%)'),y=paste0('Host PC2 (',round(host_PC_eigenval$V1[2] / sum(host_PC_eigenval$V1) * 100,2),'%)'),color = 'Platform') 

ggplot(data = data.frame(N_PCs = 1:20,Var_Explained = host_PC_eigenval$V1 / sum(host_PC_eigenval$V1))) + geom_bar(aes(x=N_PCs,y=Var_Explained),stat="identity") + ylab('Variance Explained') + xlab('PC')
