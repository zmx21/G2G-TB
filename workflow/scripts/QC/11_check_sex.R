tb_dar_pheno <- data.table::fread('../../../pheno/metadata_Sinergia_final_all_pats.txt') %>% dplyr::mutate(PATIENT_ID = as.character(PATIENT_ID))
...
Batch1_Sex_Check <- data.table::fread('../Batch1/Sex_Est_Table.txt') %>% dplyr::select(PATIENT_ID=`Sample ID`,SNPSEX = Gender) %>% dplyr::mutate(SNPSEX = toupper(SNPSEX),Source = 'Batch1')
...
Batch2_Sex_Check <- data.table::fread('../Batch2/Sex_Est_Table.txt') %>% dplyr::select(PATIENT_ID=`Sample ID`,SNPSEX = Gender) %>% dplyr::mutate(SNPSEX = toupper(SNPSEX),Source = 'Batch2')
...
data.table::fwrite(sex_mismatch %>% dplyr::select(FID=PLINK_ID,IID=PLINK_ID),'../../../Genotyping_WGS/QC/Sex_Mismatch.txt',sep = ' ',row.names = F,quote = F)
